...
- 程序默认使用 1280×720 取流、960 宽度检测，优先保证实时性；程序启动时会输出实际分辨率。
...
```bash
python tools/generate_synthetic_dataset.py --source path/to/九宫格照片.png --fruit-multiplier 3 --appliance-multiplier 2 --clean
```

这个脚本会从九宫格中裁出 9 张基准卡片，再生成旋转、亮度、模糊、透视和轻微偏移后的单张卡片样本。水果类会生成更多样本，并额外加入橙子偏黄、香蕉偏橙/偏暗、低清和轻微模糊的困难样本，用于缓解香蕉不稳定、橙子和香蕉混淆的问题；空调和冰箱也会生成更多低对比度样本，用于减少二者混淆。
